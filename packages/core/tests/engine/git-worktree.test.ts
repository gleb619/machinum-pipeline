...
function makeTargetContext(): TargetContext {
  return {
    run: {
      runId: 'test-run',
      pipelineId: 'test-pipeline',
...
function mockSource<T>(envelopes: Envelope<T>[], lifestyle: Lifecycle = 'long-lived'): Source<T> {
  return {
    uri: 'mock://source',
    lifestyle,
...
function mockTarget<T>(): Target<T> & { written: Envelope<T>[]; opened: boolean; closed: boolean } {
  const t = {
    uri: 'mock://target',
...
describe('GitWorktreeSource', () => {
  ...
  it('delegates start() to the inner source inside the worktree directory', async () => {
    ...
    const results: Envelope<string>[] = []
    for await (const env of wrapper.start(ctx)) {
      results.push(env as Envelope<string>)
    }
...
describe('createGitWorktreeSource (composite URI)', () => {
  let repoRoot: string
  let originalSourceFactory: ReturnType<typeof registry.getSourceFactory>
...
describe('createGitWorktreeTarget (composite URI)', () => {
  let repoRoot: string
  let originalTargetFactory: ReturnType<typeof registry.getTargetFactory>
