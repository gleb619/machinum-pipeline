...
import type { CacheEntry, CacheOptions } from '../../src/engine/cache.js'
...
describe('UC-19 — Tool cache (architectural)', () => {
  ...
  it('computeKey produces a deterministic SHA-256 hex string', () => {
    ...
    const opts2: CacheOptions = {
      toolName: 'test',
      version: '1.0.0',
      input: { x: 1 },
    }
    const opts3: CacheOptions = {
      toolName: 'test',
      version: '1.0.0',
      input: { x: 2 },
    }
  ...
  it('set / has / get round-trips values', async () => {
    ...
    const result = await cache.get<{ echoed: string }>(key)
  ...
  it('Tool interface supports cacheable and idempotent flags', () => {
    const tool: Pick<Tool<unknown, unknown>, 'cacheable' | 'idempotent'> = {
      cacheable: true,
      idempotent: true,
    }
